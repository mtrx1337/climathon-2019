...
load(file ="outputs/data_close_gap_raw.RData")
```
...
> if intersection -> assign severity level 1, else 0

> for production, unionize neighbouring Uber segements
...
> reducing subset to relieve frontend for demo
...
# save(close_gapbase, file = "outputs/final_full_index.RData")
```
...
> (Un)HappyBike Segments (in km) (for Level < 4)
...
> Index Segments (in km) (for Level < 4)

```{r}
close_gapindex %>% 
  summarise(n = n(), total_length_index_km = sum(length_m)/1000,
